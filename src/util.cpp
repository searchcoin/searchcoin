...
const char * const BITCOIN_CONF_FILENAME = "searchcoin.conf";
const char * const BITCOIN_PID_FILENAME = "searchcoin.pid";
...
#else
    const char* pszModule = "searchcoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Searchcoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Searchcoin";
#else
    // Unix
    return pathRet / ".searchcoin";
#endif
#endif
}
