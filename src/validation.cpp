...
#if defined(NDEBUG)
# error "Searchcoin cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "Searchcoin Signed Message:\n";
using namespace std;
// Internal stuff
namespace {
                              ...
                              bool fOverrideMempoolLimit, const CAmount& nAbsurdFee, std::vector<uint256>& vHashTxnToUncache)
{
...

    {
        ...
        {
            LogPrint("mempool", "replacing tx %s with %s for %s SSC additional fees, %d delta bytes\n",
...
{
    if (!fAddressIndex)
        return error("[%s:%d] address index not enabled",__FILE__,__LINE__);
...
bool CVerifyDB::VerifyDB(const CChainParams& chainparams, CCoinsView *coinsview, int nCheckLevel, int nCheckDepth)
{
    ...
    if (nCheckDepth <= 0)
        // Searchcoin: suffices until year 10214. Didn't x4 value due to integer wrap around and upstream compatibility.
